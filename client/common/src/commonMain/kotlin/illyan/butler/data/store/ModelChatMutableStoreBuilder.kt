...
@Single
class ModelChatMutableStoreBuilder(
    databaseHelper: DatabaseHelper,
    chatNetworkDataSource: ChatNetworkDataSource
) {
    @OptIn(ExperimentalStoreApi::class)
    val store = provideModelChatMutableStore(databaseHelper, chatNetworkDataSource)
}

@OptIn(ExperimentalStoreApi::class)
fun provideModelChatMutableStore(
    databaseHelper: DatabaseHelper,
    chatNetworkDataSource: ChatNetworkDataSource
) = MutableStoreBuilder.from(
    fetcher = Fetcher.ofFlow { key ->
        Napier.d("Fetching chats for model $key")
        chatNetworkDataSource.fetchByModel(modelUUID = key)
    },
    sourceOfTruth = SourceOfTruth.of(
        reader = { key: String ->
            databaseHelper.queryAsListFlow {
                Napier.d("Reading chats for model $key")
        ...
        writer = { key, local ->
            databaseHelper.withDatabase { db ->
                local.forEach {
                    Napier.d("Writing chat for model $key with $local")
        ...
        delete = { key ->
            databaseHelper.withDatabase {
                Napier.d("Deleting chat for model $key")
                it.chatQueries.deleteAllChatsForModel(key)
            }
        },
        deleteAll = {
            databaseHelper.withDatabase {
    ...
    bookkeeper = provideBookkeeper(
        databaseHelper,
        DomainChat::class.simpleName.toString() + "ModelList"
    ) { it }
)